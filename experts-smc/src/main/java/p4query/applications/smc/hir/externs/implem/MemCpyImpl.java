...
public class MemCpyImpl extends ExternDefinition {

    public MemCpyImpl(Declaration iface) throws UnableToLinkDeclaration {
        super(iface, "memcpy", "stdlib");
    }
    ...
    public List<StackInstruction> compileToLIR(GlobalMemoryLayout global) {
        ...
        StackInstruction popLen = new Pop();
        ...
        insts.add(new IfEq(new UnresolvedInstructionLabel(popLen, "jump if length is 0"))); // pops
        ...
        // write src bit to dst
        insts.add(new PutField()); // also pops src bit and dst

        insts.add(new Dec()); // decrease length 
        insts.add(new Goto(new UnresolvedInstructionLabel(top, "loop")));

        insts.add(popLen); // pop length
