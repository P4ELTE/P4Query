...
public class PacketIn implements Composite {
    public static final int PTR_SIZE = 1;  // don't change this, it is dependent on the stack machine

    private LinkedHashMap<String, IRType> fields = new LinkedHashMap<>();

    private int bufferSize;

    public PacketIn(int bufferSize, IRType.SingletonFactory factory){
        this.bufferSize = bufferSize;
        GenType ptrType = factory.create("PACKET_PTR", PTR_SIZE, null);
        fields.put("cursor", ptrType);

        GenType pbType = factory.create("PACKET_BUFFER", bufferSize, null);
