...
public class IsValidImpl extends ExternDefinition {

    public IsValidImpl(Declaration iface) throws UnableToLinkDeclaration {
        super(iface, "isValid", null);
    }
    ...
    public List<StackInstruction> compileToLIR(GlobalMemoryLayout global) {
        ...
        insts.add(super.openDefinition("hdr"));

        insts.add(new Load(new LocalAddress(0, "hdr.valid"))); // pointer to a header
        insts.add(new GetField());
 

        StackInstruction exit0 = new GetField(); // validity
        insts.add(exit0);
